...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlPiercing(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            int totIdx;
            double[] BodyLongPeriodTotal = new double[2];
            if (startIdx < 0)
        ...
        Label_0253:
            if (((inClose[i - 1] < inOpen[i - 1]) ? -1 : 1) == -1)
            {
                ...
                if ((Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) > ((Globals.candleSettings[0].factor * num24) / num18)) && (inClose[i] >= inOpen[i]))
                {
                    double num11;
                    double num17;
                    if (Globals.candleSettings[0].avgPeriod != 0.0)
                    ...
                    if (((Math.Abs((float)(inClose[i] - inOpen[i])) > ((Globals.candleSettings[0].factor * num17) / num11)) && (inOpen[i] < inLow[i - 1])) && ((inClose[i] < inOpen[i - 1]) && (inClose[i] > (inClose[i - 1] + (Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) * 0.5)))))
                    {
                        outInteger[outIdx] = 100;
                        outIdx++;
                        goto Label_05A6;
