...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlLongLeggedDoji(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num15;
        ...
        Label_066B:
            ...
            if (Globals.candleSettings[4].rangeType == RangeType.RealBody)
            {
                num5 = Math.Abs((float)(inClose[ShadowLongTrailingIdx] - inOpen[ShadowLongTrailingIdx]));
            }
            else
            {
                float num4;
