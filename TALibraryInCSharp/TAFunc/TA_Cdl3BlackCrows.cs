...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode Cdl3BlackCrows(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            int totIdx;
            double[] ShadowVeryShortPeriodTotal = new double[3];
            if (startIdx < 0)
