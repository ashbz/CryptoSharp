...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode Cdl3StarsInSouth(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num25;
        ...
        Label_0572:
            if (((((inClose[i - 2] < inOpen[i - 2]) ? -1 : 1) == -1) && (((inClose[i - 1] < inOpen[i - 1]) ? -1 : 1) == -1)) && (((inClose[i] < inOpen[i]) ? -1 : 1) == -1))
            {
                ...
                if (Math.Abs((float)(inClose[i - 2] - inOpen[i - 2])) > ((Globals.candleSettings[0].factor * num86) / num80))
                {
                    ...
                    if (((((num79 - inLow[i - 2]) > ((Globals.candleSettings[4].factor * num78) / num72)) && (Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) < Math.Abs((float)(inClose[i - 2] - inOpen[i - 2])))) && ((inOpen[i - 1] > inClose[i - 2]) && (inOpen[i - 1] <= inHigh[i - 2]))) && ((inLow[i - 1] < inClose[i - 2]) && (inLow[i - 1] >= inLow[i - 2])))
                    {
                        double num64;
                        double num70;
                        float num71;
