...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlHomingPigeon(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num15;
        ...
        Label_026B:
            if ((((inClose[i - 1] < inOpen[i - 1]) ? -1 : 1) == -1) && (((inClose[i] < inOpen[i]) ? -1 : 1) == -1))
            {
                ...
                if (Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) > ((Globals.candleSettings[0].factor * num34) / num28))
                {
                    ...
                    if (((Math.Abs((float)(inClose[i] - inOpen[i])) <= ((Globals.candleSettings[2].factor * num27) / num21)) && (inOpen[i] < inOpen[i - 1])) && (inClose[i] > inClose[i - 1]))
                    {
                        outInteger[outIdx] = 100;
                        outIdx++;
                        goto Label_0588;
