...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode AdOsc(int startIdx, int endIdx, float[] inHigh, float[] inLow, float[] inClose, float[] inVolume, int optInFastPeriod, int optInSlowPeriod, ref int outBegIdx, ref int outNBElement, double[] outReal)
        {
            int slowestPeriod;
            if (startIdx < 0)
            {
