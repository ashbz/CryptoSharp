...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlConcealBabysWall(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            int totIdx;
            double[] ShadowVeryShortPeriodTotal = new double[4];
            if (startIdx < 0)
