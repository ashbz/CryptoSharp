...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlTasukiGap(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            int num11;
        ...
        Label_0176:
            ...
            if (((((num31 > num30) && (inClose[i - 1] >= inOpen[i - 1])) && (((inClose[i] < inOpen[i]) ? -1 : 1) == -1)) && ((inOpen[i] < inClose[i - 1]) && (inOpen[i] > inOpen[i - 1]))) && (inClose[i] < inOpen[i - 1]))
            {
                ...
                if (inClose[i] > num29)
                {
                    ...
                    if (Math.Abs((float)(Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) - Math.Abs((float)(inClose[i] - inOpen[i])))) < ((Globals.candleSettings[8].factor * num28) / num22))
                    {
                        goto Label_063F;
                    }
                }
            ...
            if (Math.Abs((float)(Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) - Math.Abs((float)(inClose[i] - inOpen[i])))) >= ((Globals.candleSettings[8].factor * num18) / num12))
            {
                goto Label_0665;
            }
        Label_063F:
