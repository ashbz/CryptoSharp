...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlDarkCloudCover(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, double optInPenetration, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            if (startIdx < 0)
        ...
        Label_01B1:
            if (inClose[i - 1] >= inOpen[i - 1])
            {
                ...
                if ((((Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) > ((Globals.candleSettings[0].factor * num17) / num11)) && (((inClose[i] < inOpen[i]) ? -1 : 1) == -1)) && ((inOpen[i] > inHigh[i - 1]) && (inClose[i] > inOpen[i - 1]))) && (inClose[i] < (inClose[i - 1] - (Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) * optInPenetration))))
                {
                    outInteger[outIdx] = -100;
                    outIdx++;
                    goto Label_03A1;
