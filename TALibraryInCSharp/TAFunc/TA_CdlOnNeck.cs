...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlOnNeck(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num15;
            ...
            while (true)
            {
                ...
                if (Globals.candleSettings[10].rangeType == RangeType.RealBody)
                {
                    num51 = Math.Abs((float)(inClose[i - 1] - inOpen[i - 1]));
                }
                else
                {
                    float num50;
        ...
        Label_028E:
            if (((inClose[i - 1] < inOpen[i - 1]) ? -1 : 1) == -1)
            {
                ...
                else
                {
                    ...
                    if (Globals.candleSettings[0].rangeType == RangeType.RealBody)
                    {
                        num40 = Math.Abs((float)(inClose[i - 1] - inOpen[i - 1]));
                    }
                    else
                    {
                        float num39;
                ...
                if (((Math.Abs((float)(inClose[i - 1] - inOpen[i - 1])) > ((Globals.candleSettings[0].factor * num41) / num35)) && (inClose[i] >= inOpen[i])) && (inOpen[i] < inLow[i - 1]))
                {
                    double num28;
                    double num34;
                    if (Globals.candleSettings[10].avgPeriod != 0.0)
