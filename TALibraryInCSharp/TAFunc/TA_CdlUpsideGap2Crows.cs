...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlUpsideGap2Crows(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num15;
        ...
        Label_0254:
            if (inClose[i - 2] >= inOpen[i - 2])
            {
                ...
                if ((Math.Abs((float)(inClose[i - 2] - inOpen[i - 2])) > ((Globals.candleSettings[0].factor * num36) / num30)) && (((inClose[i - 1] < inOpen[i - 1]) ? -1 : 1) == -1))
                {
                    double num23;
                    double num29;
                    if (Globals.candleSettings[2].avgPeriod != 0.0)
