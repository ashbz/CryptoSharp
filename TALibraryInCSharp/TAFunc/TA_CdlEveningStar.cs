...
namespace TALibraryInCSharp
     ...
     { 
        ...
        public static RetCode CdlEveningStar(int startIdx, int endIdx, float[] inOpen, float[] inHigh, float[] inLow, float[] inClose, double optInPenetration, ref int outBegIdx, ref int outNBElement, int[] outInteger)
        {
            float num5;
            float num10;
            float num15;
        ...
        Label_037D:
            ...
            if ((Math.Abs((float)(inClose[i - 2] - inOpen[i - 2])) > ((Globals.candleSettings[0].factor * num53) / num47)) && (inClose[i - 2] >= inOpen[i - 2]))
            {
                double num40;
                double num46;
                if (Globals.candleSettings[2].avgPeriod != 0.0)
